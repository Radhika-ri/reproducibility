...
water.imbibed.cor <- ggplot(sample.data.bac.nosoil, aes(y = even, x = 1000 * Water_Imbibed, color = Crop)) +  # Define aesthetics: y-axis as even, x-axis as Water_Imbibed (converted to mg), and color by Crop
  ...
  geom_smooth(se = FALSE, method = lm) +  # Add a linear model smooth line without confidence interval shading, TREND LINE
  ...
  scale_shape_manual(values = c(15, 16, 17, 18), name = "", labels = c("0 hrs", "6 hrs", "12 hrs", "18 hrs")) +  # WE CAN CHECK THE SHAPES FROM THE GOOGLE, NUMBER REPRESENT THE DIFFERENT SHAPES,  Manually set shapes for the Time.Point variable
  theme_classic() +  # Use a classic theme for the plot
...
# JUST LIKE PAPER, STITCHING ALL FIGURES IN ONE, Arrange multiple ggplot objects into a single figure
figure2 <- ggarrange(
...
#add significant or non significant 
bac.even + 
